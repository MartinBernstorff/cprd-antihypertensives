...
class CausalCohort:
    ...
    def date_least_year_register_gp(self, demographics, death):
        ...
        time = time.withColumn(
            ...
            time.startdate + F.expr(f"INTERVAL {self.least_year_register_gp} YEAR"),
        )
        demographics = (
            demographics.join(time, on="patid", how="inner")
            .drop(demographics.enddate)
            .drop(demographics.startdate)
        )
        return demographics
    ...
    def date_least_age(self, demographics):
        ...
        demographics = demographics.withColumn(
            ...
            demographics.dob + F.expr(f"INTERVAL {self.least_age} YEARS"),
        )
        return demographics
    ...
    def date_greatest_age(self, demographics):
        ...
        demographics = demographics.withColumn("dob", cvt_str2time(demographics, "dob"))
        demographics = demographics.withColumn(
            f"{self.greatest_age}_dob",
            demographics.dob + F.expr(f"INTERVAL {self.greatest_age} YEARS"),
        )
        return demographics
    ...
    def standard_prepare(self, file, spark, linkage=True, practiceLink=True):
        ...
        demographics = self.retrieve_eligible_patid(
            patient,
            practice,
            eligible,
            linkage,
...
class CohortSoftCut(CausalCohort):
    ...
    def demoExtract(self, file, spark, duration=("1995-01-01", "2010-01-01")):
        ...
        demographics = (
            ...
            .drop("study_entry")
            .withColumnRenamed("study_entry_real", "study_entry")
        )
        demographics = demographics.withColumn(
            "exit_date",
        ...
        demographics = (
            demographics.withColumn(
                ...
                F.least(
                    F.col("exit_date"),
                    F.to_date(F.lit(duration[1])),
                    F.col(f"{self.greatest_age}_dob"),
                ),
            )
            .drop("exit_date")
            .withColumnRenamed("exit_datereal", "exit_date")
        )
    ...
    def pipeline(
        ...
        file,
        spark,
        duration=("1995-01-01", "2010-01-01"),
        randomNeg=True,
        sourceT=None,
        sourceCol=None,
        rollingTW=-1,
        ...
        demographics = demographics.drop("study_entry").withColumnRenamed(
            "eventdate",
            "study_entry",
        )
        return demographics
    ...
    def randomizeNeg(self, file, spark, demographics, randomNeg=True):
        ...
        if randomNeg:
            ...
            neg = (
                neg.withColumn(
                    "diff",
                    F.datediff(F.col("exit_date"), F.col("study_entry")),
                )
                .withColumn("diff", rand_generate("diff"))
                .withColumn("eventdate", F.expr("date_add(study_entry, diff)"))
                .drop("diff")
            )
    ...
    def get_exp(self, demographics, source, condition, column="code", rollingTW=-1):
        ...
        if rollingTW == -1:
            ...
            source_first = (
                source.withColumn(column, F.first(column).over(w))
                .groupBy("patid")
                .agg(
            ...
            demographics = demographics.join(
                ...
                "patid",
                "left",
            ).withColumnRenamed(column, "expCode")
            ...
            demographics = (
                ...
                .join(
                    exclude.select(["patid", "eventdate"]).alias("b"),
                    F.col("a.patid") == F.col("b.patid"),
                    "left",
                )
                .filter(F.col("b.patid").isNull())
                .select("a.*")
            )
        ...
        # no need to worry about this for now...
            ...
            def monthsLambda(i):
                return int(-i * 30 * 86400)

            # Considering the dataframe already created using code provided in question
            source2 = source2.withColumn(
                "unix_time",
                F.unix_timestamp("eventdate", "yyyy-MM-dd"),
            )
            source2startdate = source2.groupBy("patid").agg(
                F.min("eventdate").alias("first_drugdate"),
            )
            winSpec = (
                Window.partitionBy("patid")
                .orderBy("unix_time")
                .rangeBetween(monthsLambda(rollingTW), 0)
            )
            winSpec2 = (
                Window.partitionBy("patid")
                .orderBy("unix_time")
                .rangeBetween(monthsLambda(rollingTW + 13), 0)
            )
            ...
            rwDiff = F.unix_timestamp("maxdate", "yyyy-MM-dd") - F.unix_timestamp(
                "mindate",
                "yyyy-MM-dd",
            )
            source2 = source2.withColumn("rwDiff", rwDiff).withColumn(
                "rwDiff",
                (F.col("rwDiff") / 3600 / 24 / 30).cast("integer"),
            )
            #

            rwDiff = F.unix_timestamp("maxdate2", "yyyy-MM-dd") - F.unix_timestamp(
                "mindate2",
                "yyyy-MM-dd",
            )
            source2 = source2.withColumn("rwDiff2", rwDiff).withColumn(
                "rwDiff2",
                (F.col("rwDiff2") / 3600 / 24 / 30 / (rollingTW + 12)).cast("integer"),
            )
            #

            source2 = source2.select(
                ["patid", "eventdate", "prodcode", "rwDiff", "rwDiff2", "mindate"],
            )
            source2del = (
                source2.where(F.col("rwDiff") < rollingTW - 1)
                .select("patid")
                .dropDuplicates()
            )
            source2keep = (
            ...
            source2del = (
                source2del.alias("a")
                .join(
                    source2keep.select(["patid"]).alias("b"),
                    F.col("a.patid") == F.col("b.patid"),
                    "left",
                )
                .filter(F.col("b.patid").isNull())
            ...
            source_first = (
                source2keep2.withColumn(column, F.first(column).over(w))
                .groupBy("patid")
                .agg(
                    F.min("mindate").alias("eventdate"),
                    F.first("prodcode").alias("prodcode"),
                    F.first("rwDiff").alias("rwDiff"),
                    F.min("first_drugdate").alias("first_drugdate"),
                    F.first("rwDiff2").alias("rwDiff2"),
                )
            )

            source_first2keep = source_first.filter(
                (F.col("eventdate") == F.col("first_drugdate"))
                | (F.col("rwDiff2") > 0),
            )
            ...
            demographics = (
                demographics.alias("z")
                .join(
                    source2fulldel.select(["patid"]).alias("b"),
            ...
            demographics = (
                demographics.alias("z")
                .join(
                    source_first2del.select(["patid"]).alias("b"),
                    F.col("z.patid") == F.col("b.patid"),
                    "left",
                )
                .filter(F.col("b.patid").isNull())
        ...
        positive = demographics.where(
            F.col("eventdate") <= F.col("exit_date"),
        ).withColumn("exp_label", F.lit(1))
        ...
        negative = (
            demographics.alias("a")
            .join(
                positive.select(["patid", "eventdate"]).alias("b"),
                F.col("a.patid") == F.col("b.patid"),
                "left",
            )
            .filter(F.col("b.patid").isNull())
