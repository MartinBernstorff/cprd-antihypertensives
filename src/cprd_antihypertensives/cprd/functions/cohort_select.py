...
class CohortHardCut(Cohort):
    ...
    def pipeline(self, file, spark, study_entry_date="1995-01-01"):
        ...
        demographics = demographics.where(
            F.col("study_entry") > F.col(f"{self.least_age}_dob")
        )
        demographics = demographics.where(
            F.col("study_entry") <= F.col(f"{self.greatest_age}_dob")
        )

        # study entry date after x year of registration with gp
        demographics = demographics.where(
            F.col("study_entry") > F.col("least_gp_register_date")
        )
...
class CohortSoftCut(Cohort):
    ...
    def pipeline(self, file, spark, duration=("1995-01-01", "2015-01-01")):
        ...
        ).drop("exit_date").withColumnRenamed("exit_datereal", "exit_date")
...
class CohortRandomCut(Cohort):
    ...
    def pipeline(self, file, spark, duration=("1995-01-01", "2015-01-01")):
        ...
        demographics = demographics.withColumn(
            "start",
            F.greatest(
                F.col("least_gp_register_date"),
                F.col(f"{self.least_age}_dob"),
        ...
        demographics = (
            demographics.withColumn("diff", F.datediff(F.col("end"), F.col("start")))
            .withColumn("diff", rand_generate("diff"))
            .withColumn("study_entry", F.expr("date_add(start, diff)"))
            .drop("diff")
        )
