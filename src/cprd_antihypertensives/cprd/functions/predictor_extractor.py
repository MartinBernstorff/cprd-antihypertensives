...
class PredictorExtractorBase:
    def predictor_extract(
        ...
        col,
        colname="code",
        col_baseline="study_entry",
        span_before_baseline_month=None,
        type="last",
    ):
        """
        ...
        if span_before_baseline_month is not None:
            df = df.withColumn(
                "span_date",
                df[col_baseline]
                - F.expr(f"INTERVAL {span_before_baseline_month} MONTH"),
            )
        ...
        if type == "last":
            df = (
                df.withColumn(col, F.last(col).over(w))
                .groupBy("patid")
                .agg(F.last(col).alias(col))
            )
        elif type == "var":
            df = (
                df.withColumn(col, F.last(col).over(w))
                .groupBy("patid")
                .agg(F.variance(col).alias(col))
            )
        elif type == "mean":
            df = (
                df.withColumn(col, F.last(col).over(w))
                .groupBy("patid")
                .agg(F.mean(col).alias(col))
            )
        elif type == "std":
            df = (
                df.withColumn(col, F.last(col).over(w))
                .groupBy("patid")
                .agg(F.stddev(col).alias(col))
            )
        else:
    ...
    def predictor_check_exist(
        ...
        if countsBased == -1:
            df = (
                df.filter(F.col(col).isin(*condition))
                .select(["patid"])
                .dropDuplicates()
                .withColumn("exist", F.lit(1))
            )
        else:
            df = (
                df.filter(F.col(col).isin(*condition))
    ...
    def flatten(self, file, spark, df, code_column="code", add_sep=True):
        ...
        df = df.groupby(["patid", "eventdate"]).agg(
            F.collect_list(code_column).alias(code_column),
        ...
        if add_sep:
            ...
            df = (
                df.withColumn("age_temp", extract_age("age"))
                .withColumn("age", F.concat(F.col("age"), F.array(F.col("age_temp"))))
                .drop("age_temp")
            )
        w = Window.partitionBy("patid").orderBy("eventdate")
        df = (
            df.withColumn(code_column, F.collect_list(code_column).over(w))
            .withColumn("age", F.collect_list("age").over(w))
            .groupBy("patid")
            .agg(F.max(code_column).alias(code_column), F.max("age").alias("age"))
        )
...
class BEHRT(PredictorExtractorBase):
    def format_behrt(
        ...
        data = (
            data.join(demorgraphics, "patid", "inner")
            .dropna()
    ...
    def _format_sequence(self, data, col_code, col_age):
        # group by date
        data = data.groupby(["patid", "eventdate"]).agg(
            F.collect_list(col_code).alias(col_code),
            F.collect_list(col_age).alias(col_age),
        ...
        extract_age = F.udf(lambda x: x[0])
        data = (
            data.withColumn("age_temp", extract_age(col_age))
            .withColumn(col_age, F.concat(F.col(col_age), F.array(F.col("age_temp"))))
            .drop("age_temp")
        )
        ...
        w = Window.partitionBy("patid").orderBy("eventdate")
        data = (
            data.withColumn(col_code, F.collect_list(col_code).over(w))
            .withColumn(col_age, F.collect_list(col_age).over(w))
            .groupBy("patid")
            .agg(
                F.max(col_code).alias(col_code),
                F.max(col_age).alias(col_age),
                F.first("label").alias("label"),
            )
        )
        data = (
            EHR(data).array_flatten(col_code).array_flatten(col_age)
        )  # patid, code, age
        return data
...
class BEHRTCausal(PredictorExtractorBase):
    ...
    def _format_sequence(self, data, col_code, col_age, col_year, explabel):
        ...
        data = data.groupby(["patid", "eventdate"]).agg(
            F.collect_list(col_code).alias(col_code),
            F.collect_list(col_age).alias(col_age),
            F.collect_list(col_year).alias(col_year),
            F.first("label").alias("label"),
            F.first(explabel).alias(explabel),
        )
        data = EHR(data).array_add_element(col_code, "SEP")
        # add extra age to fill the gap of sep
        extract_age = F.udf(lambda x: x[0])
        ...
        data = (
            data.withColumn("year_temp", extract_age("year"))
            .withColumn("year", F.concat(F.col("year"), F.array(F.col("year_temp"))))
            .drop("year_temp")
        )
        # sort and merge code and age
        ...
        data = (
            EHR(data)
            .array_flatten(col_code)
            .array_flatten(col_age)
            .array_flatten("year")
        )  # patid, code, age
        return data
