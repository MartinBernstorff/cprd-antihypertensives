...
import cprd_antihypertensives.cprd.base.table as cprd_table
from cprd_antihypertensives.cprd.config.spark import *
from cprd_antihypertensives.cprd.config.utils import *
from cprd_antihypertensives.cprd.functions import MedicalDictionary, merge, tables
...
def retrieve_hdlr(file, spark, duration=(1985, 2021), usable_range=(0, 10)):
    ...
    """
    condition_query = MedicalDictionary.MedicalDictionaryRiskPrediction(file, spark)
    hdlr = condition_query.queryMeasurement(["cholesterol"], merge=True)["cholesterol"][
        "medcode"
    ]
...
def retrieve_smoking_status(file, spark, duration=(1985, 2021)):
    ...
    """
    condition_query = MedicalDictionary.MedicalDictionaryRiskPrediction(file, spark)
    smoke = condition_query.queryMeasurement(["smoking+yes"], merge=True)[
        "smoking+yes"
    ]["medcode"]
    ex_smoke = condition_query.queryMeasurement(["smoking+ex"], merge=True)[
        "smoking+ex"
    ]["medcode"]
    no_smoke = condition_query.queryMeasurement(["smoking+no"], merge=True)[
        "smoking+no"
    ]["medcode"]
...
def retrieve_heartrate_measurement(
    ...
    """
    condition_query = MedicalDictionary.MedicalDictionaryRiskPrediction(file, spark)
    hr = condition_query.queryMeasurement(["heart rate"], merge=True)["heart rate"][
        "medcode"
    ]
...
def retrieve_creatinine_measurement(
    ...
    """
    condition_query = MedicalDictionary.MedicalDictionaryRiskPrediction(file, spark)
    creat = condition_query.queryMeasurement(["creatinine"], merge=True)["creatinine"][
        "medcode"
    ]
...
def retrieve_imd(file, spark):
    ...
    imd = (
        imd.select(["patid", "imd2015_5"])
        .filter(imd.imd2021_5 != "")
...
def retrieve_test_LEGACY(file, spark, duration=(1985, 2021)):
    ...
    test = (
        test.withColumn("tests", translate(lookup)("enttype"))
        .na.drop()
        .select(["patid", "eventdate", "tests"])
    )
