...
class PredBase:
    ...
    def exclusion_inclusion_record(
        ...
        source = (
            source.filter(F.col(column).isin(*criteria))
            .withColumnRenamed(column, "code")
            .select(["patid", "eventdate", "code"])
        )
        ...
        subset = (
            subset.where(F.col("eventdate") <= F.col("study_entry"))
            .withColumn("redundant", F.lit(1))
            .select(["patid", "redundant"])
            .dropDuplicates()
        )
    ...
    def _check_follow_up_duration(self, demographics):
        ...
        demographics = demographics.withColumn(
            "endFollowUp",
            demographics.study_entry
            + F.expr(f"INTERVAL {self.follow_up_duration} MONTHS"),
        )

        # if an event happens before the end of follow up, then a positive case is identified
        positivePOSSIBLE = demographics.where(
            F.col("eventdate") <= F.col("endFollowUp"),
        )

        positive = positivePOSSIBLE.where(
            F.col("eventdate") <= F.col("enddate"),
        ).withColumn("label", F.lit(1))
        moreNEG = positivePOSSIBLE.where(
            F.col("eventdate") > F.col("enddate"),
        ...
        negtive_1 = demographics.where(
            F.col("eventdate") > F.col("endFollowUp"),
        ).withColumn("label", F.lit(0))
        negtive_1BEFORE = (
            negtive_1.where(F.col("endFollowUp") <= F.col("enddate"))
            .withColumn("time2event", F.lit(self.time2eventMarkDefault))
            .select(["patid", "label", "time2event"])
        )
        negtive_1AFTER = (
            negtive_1.where(F.col("endFollowUp") > F.col("enddate"))
        ...
        unclear = (
            ...
            .join(identified.alias("b"), F.col("a.patid") == F.col("b.patid"), "left")
            .filter(F.col("b.patid").isNull())
            .select("a.*")
        )
        negative = (
            unclear.where(F.col("endFollowUp") < F.col("enddate"))
            .withColumn("label", F.lit(0))
            .withColumn("time2event", F.lit(self.time2eventMarkDefault))
            .select(["patid", "label", "time2event"])
        )
    ...
    def get_label_from_records(
        ...
        if incidence:
            ...
            if prevalent_conditions is None:
                ...
                demorawcolumns = [
                    colsdemo
                    for colsdemo in demographics.columns
                    if colsdemo not in ["eventdate", "code"]
                ]
                ...
                demographics = (
                    demographics.alias("a")
                    .join(
                        excludePotentialPats.select(["patid"]).alias("b"),
                        F.col("a.patid") == F.col("b.patid"),
                        "left",
                    )
                    .filter(F.col("b.patid").isNull())
                ...
                w = Window.partitionBy("patid").orderBy("eventdate")
                demographicsDates = (
                    demographicsDates.withColumn(column, F.first(column).over(w))
                    .groupBy("patid")
                    .agg(
                        F.min("eventdate").alias("eventdate"),
                        F.first(column).alias(column),
    ...
    def get_label_from_death_registration(self, demographics, death, condition):
        ...
        death = (
            death.withColumn("cause", F.explode("cause"))
            .withColumn("cause", rm_dot("cause"))
            .filter(F.col("cause").isin(*condition))
        )
        death = (
            death.groupBy("patid")
            .agg(F.first("cause").alias("cause"))
            .withColumn("label", F.lit(1))
            .select(["patid", "label"])
        )
...
class OutcomePrediction(PredBase):
    ...
    def exclusion_inclusion_diagnosis_medication(
        self, demographics, source=None, source_col="code", exclusion_source=True,
    ):
        ...
        if self.exclusion_codes is not None:
            if source is None:
                raise ValueError(
                    "exclusion criteria for proper source is not None, provide source (i.e., diag, meds, etc) dataframe",
                )
            else:
                for _diag, code in self.exclusion_codes.items():
                    demographics, _ = self.exclusion_inclusion_record(
                        demographics,
                        code,
                        source,
                        source_col,
    ...
    def pipeline(
        ...
        demographics, _ = self.exclusion_inclusion_diagnosis_medication(
            demographics, source, column_condition, exclusion_source,
        )
        # set up label for demographics
        demographics = self.set_label(
            demographics,
            source,
