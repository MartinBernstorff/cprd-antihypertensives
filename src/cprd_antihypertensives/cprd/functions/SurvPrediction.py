...
class SurvRiskPredictionBase:
    ...
    def setupEventAndTime(self, demographics, source, condition, column, death=None):
        ...
        demographics_no_event = (
            demographics.filter(F.col("eventdate").isNull())
            .withColumn("event", F.lit(0))
            .withColumn("time", F.least(F.col("enddate"), F.col("endfollowupdate")))
        )

        demographics_with_event = (
            demographics.filter(F.col("eventdate").isNotNull())
            .filter(F.col("eventdate") > F.col("study_entry"))
            .cache()
        )
        ...
        demographics = (
            demographics_no_event.union(demographics_with_event_a)
            .union(demographics_with_event_b)
